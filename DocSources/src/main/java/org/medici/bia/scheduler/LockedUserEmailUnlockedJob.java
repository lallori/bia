/*
 * LockedUserEmailUnlockedJob.java
 *
...
public class LockedUserEmailUnlockedJob {
	@Autowired
	...
	public void execute() {
		MDC.put("account", "threadunlockeduser");
		try {
			List<LockedUser> lockedUsers = getAdminService().findUnlockedUsers();
			
			for(LockedUser currentLockedUser : lockedUsers){
				getMailService().sendMailUnlockedUser(currentLockedUser);
			}
