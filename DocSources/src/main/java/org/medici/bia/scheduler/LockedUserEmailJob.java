...
public class LockedUserEmailJob {
	@Autowired
	...
	public void execute() {
		MDC.put("account", "threadlockeduser");
		try {
			List<LockedUser> lockedUsers = getAdminService().findLockedUsers();
			
			for(LockedUser currentLockedUser : lockedUsers){
				getMailService().sendMailLockedUser(currentLockedUser);
				getAdminService().sendLockedMessage(currentLockedUser.getUser());
			}
