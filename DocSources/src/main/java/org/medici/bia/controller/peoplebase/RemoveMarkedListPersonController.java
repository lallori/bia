/*
 * RemoveMarkedListPersonController.java
 * 
...
import org.medici.bia.command.peoplebase.RemoveMarkedListPersonCommand;
...
/**
 * Controller for action "Remove marked list Person".
 * 
 * @author Lorenzo Pasquinelli (<a href=mailto:l.pasquinelli@gmail.com>l.pasquinelli@gmail.com</a>)
 * @author Matteo Doni (<a href=mailto:donimatteo@gmail.com>donimatteo@gmail.com</a>)
 */
@Controller
@RequestMapping("/src/peoplebase/RemoveMarkedListPerson")
public class RemoveMarkedListPersonController {
	@Autowired
	private PeopleBaseService peopleBaseService;
	@Autowired
	private UserMarkedListService userMarkedListService;
	@Autowired(required = false)
	@Qualifier("removeMarkedListPersonValidator")
	...
	public ModelAndView setupForm(@ModelAttribute("command") RemoveMarkedListPersonCommand command, BindingResult result){
		...
		if(!result.hasErrors()){
			...
			if(command.getPersonId() > 0){
				try {
					...
					userMarkedList = getUserMarkedListService().removePersonFromMarkedList(userMarkedList, person);
					
					model.put("person", person);
				} catch (ApplicationThrowable applicationThrowable) {
					model.put("applicationThrowable", applicationThrowable);
					new ModelAndView("response/MarkedListRemoveKO", model);
				}
			}		
		}
