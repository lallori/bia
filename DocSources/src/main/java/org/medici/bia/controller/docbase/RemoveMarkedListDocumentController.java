/*
 * RemoveMarkedListDocumentController.java
 * 
...
import org.medici.bia.command.docbase.RemoveMarkedListDocumentCommand;
...
/**
 * Controller for action "Remove marked list document".
 * 
 * @author Lorenzo Pasquinelli (<a href=mailto:l.pasquinelli@gmail.com>l.pasquinelli@gmail.com</a>)
 * @author Matteo Doni (<a href=mailto:donimatteo@gmail.com>donimatteo@gmail.com</a>)
 */
@Controller
@RequestMapping("/src/docbase/RemoveMarkedListDocument")
public class RemoveMarkedListDocumentController {
	@Autowired
	private DocBaseService docBaseService;
	@Autowired
	private UserMarkedListService userMarkedListService;
	@Autowired(required = false)
	@Qualifier("removeMarkedListDocumentValidator")
	...
	public ModelAndView processSubmit(@ModelAttribute("command") RemoveMarkedListDocumentCommand command, BindingResult result) {
		...
		if(!result.hasErrors()){		
			...
			if(command.getEntryId() > 0){
				try {
					...
					userMarkedList = getUserMarkedListService().removeDocumentFromMarkedList(userMarkedList, document);
					
					model.put("document", document);
		
				} catch (ApplicationThrowable applicationThrowable) {
					model.put("applicationThrowable", applicationThrowable);
					return new ModelAndView("response/MarkedListRemoveKO", model);
				}
			}
		}	
		model.put("category", "document");
	
		return new ModelAndView("response/MarkedListRemoveOK", model);
