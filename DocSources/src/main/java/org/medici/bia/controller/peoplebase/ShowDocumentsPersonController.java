 ...
 */
@Controller
@RequestMapping("/src/peoplebase/ShowDocumentsPerson")
	...
	public ModelAndView setupForm(@ModelAttribute("requestCommand") ShowDocumentsPersonCommand command, BindingResult result, HttpSession session) {
		...
		if (command.getPersonId() > 0) {
			try {
				...
				personId.add(person.getPersonId());
				List<String> personName = new ArrayList<String>();
				personName.add(person.getMapNameLf());
				advancedSearchDocument.setPersonId(personId);
				advancedSearchDocument.setPerson(personName);
