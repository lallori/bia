/*
 * EditForumPostAnnotationController.java
 * 
...
import org.medici.bia.command.community.EditForumPostAnnotationCommand;
...
public class EditForumPostAnnotationController {
	...
	public ModelAndView setupForm(@ModelAttribute("command") EditForumPostAnnotationCommand command) {
		Map<String, Object> model = new HashMap<String, Object>(0);

		if ((command.getTopicId() != null) && (command.getTopicId() > 0)) {
			ForumPost forumPost = new ForumPost();
			ForumTopic topicAnnotation = new ForumTopic(command.getTopicId());

			try {
				topicAnnotation = getCommunityService().getForumTopic(topicAnnotation);
