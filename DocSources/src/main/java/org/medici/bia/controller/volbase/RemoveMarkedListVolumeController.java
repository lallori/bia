/*
 * RemoveMarkedListVolumeController.java
 * 
...
/**
 * Controller for action "Remove marked list volume".
 * 
 * @author Lorenzo Pasquinelli (<a href=mailto:l.pasquinelli@gmail.com>l.pasquinelli@gmail.com</a>)
 * @author Matteo Doni (<a href=mailto:donimatteo@gmail.com>donimatteo@gmail.com</a>)
 */
@Controller
@RequestMapping("/src/volbase/RemoveMarkedListVolume")
public class RemoveMarkedListVolumeController {
	@Autowired
	private VolBaseService volBaseService;
	@Autowired
	private UserMarkedListService userMarkedListService;
	@Autowired(required = false)
	@Qualifier("removeMarkedListVolumeValidator")
	...
	public ModelAndView setupForm(@ModelAttribute("requestCommand") RemoveMarkedListVolumeCommand command, BindingResult result) {
		...
		if(!result.hasErrors()){
			...
			if(command.getSummaryId() > 0){
				try {
					...
					userMarkedList = getUserMarkedListService().removeVolumeFromMarkedList(userMarkedList, volume);
