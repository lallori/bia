/*
 * RemoveMarkedListPlaceController.java
 * 
...
/**
 * Controller for action "Remove marked list place".
 * 
 * @author Lorenzo Pasquinelli (<a href=mailto:l.pasquinelli@gmail.com>l.pasquinelli@gmail.com</a>)
 * @author Matteo Doni (<a href=mailto:donimatteo@gmail.com>donimatteo@gmail.com</a>)
 */
@Controller
@RequestMapping("/src/geobase/RemoveMarkedListPlace")
public class RemoveMarkedListPlaceController {
	@Autowired
	private GeoBaseService geoBaseService;
	@Autowired
	private UserMarkedListService userMarkedListService;
	@Autowired(required = false)
	@Qualifier("removeMarkedListPlaceValidator")
	...
	public ModelAndView setupForm(@ModelAttribute("command") RemoveMarkedListPlaceCommand command, BindingResult result) {
		...
		if(!result.hasErrors()){
			...
			if(command.getPlaceAllId() > 0){
				try {
					...
					userMarkedList = getUserMarkedListService().removePlaceFromMarkedList(userMarkedList, place);
