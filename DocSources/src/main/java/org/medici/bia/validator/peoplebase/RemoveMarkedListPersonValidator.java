/*
 * RemoveMarkedListPersonValidator.java
 * 
 ...
 * 
 */
public class RemoveMarkedListPersonValidator implements Validator {
	@Autowired
	...
	public boolean supports(Class givenClass) {
		return givenClass.equals(RemoveMarkedListPersonCommand.class);
	}
	...
	public void validate(Object object, Errors errors) {
		RemoveMarkedListPersonCommand removeMarkedListPersonCommand = (RemoveMarkedListPersonCommand) object;
		validatePersonId(removeMarkedListPersonCommand.getPersonId(), errors);
		validateRemoveMarkedList(removeMarkedListPersonCommand.getPersonId(), errors);
	}
	 ...
	 * @param errors
	 */
	public void validateRemoveMarkedList(Integer personId, Errors errors){
		if(!errors.hasErrors()){
			try{
				...
				if(userMarkedList != null){
					if(!getUserMarkedListService().ifPersonAlreadyPresent(userMarkedList, personId)){
						errors.reject("personId", "error.person.notInMarked");
					}
				}
			}catch(ApplicationThrowable ath){
				errors.reject("personId", "error.person.notInMarked");
