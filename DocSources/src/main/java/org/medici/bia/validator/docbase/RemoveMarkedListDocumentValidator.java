/*
 * RemoveMarkedListDocumentValidator.java
 * 
 ...
 * 
 */
public class RemoveMarkedListDocumentValidator implements Validator {
	@Autowired
	...
	public boolean supports(Class givenClass) {
		return givenClass.equals(RemoveMarkedListDocumentCommand.class);
	}
	...
	public void validate(Object object, Errors errors) {
		RemoveMarkedListDocumentCommand removeMarkedListDocumentCommand = (RemoveMarkedListDocumentCommand) object;
		validateDocument(removeMarkedListDocumentCommand.getEntryId(), errors);
		validateRemoveMarkedList(removeMarkedListDocumentCommand.getEntryId(), errors);
	}
	...
	public void validateRemoveMarkedList(Integer entryId, Errors errors) {
		if(!errors.hasErrors()){
			try{
				...
				if(userMarkedList != null){
					if(!getUserMarkedListService().ifDocumentAlreadyPresent(userMarkedList, entryId)){
						errors.reject("entryId", "error.document.notInMarked");
					}
				}
			}catch(ApplicationThrowable ath){
				errors.reject("entryId", "error.document.notInMarked");
