...
public class EplToLinkUtils {
	...
	public static List<Integer> getTopicIdList(List<EplToLink> inputList) {
		...
		for (int i = 0; i < inputList.size(); i++) {
			if (inputList.get(i).getTopic() != null) {
				if (! retValue.contains(inputList.get(i).getTopic().getTopicId())) {
					retValue.add(inputList.get(i).getTopic().getTopicId());
				}
			}
		}
