/*
 * ShowSenderDocumentsPersonController.java
 * 
 ...
 */
@Controller
@RequestMapping("/src/peoplebase/ShowSenderDocumentsPerson")
public class ShowSenderDocumentsPersonController {
	@Autowired
	...
	public ModelAndView setupForm(@ModelAttribute("requestCommand") ShowDocumentsPersonCommand command, BindingResult result, HttpSession session) {
		...
		if(command.getPersonId() > 0){
			try {
				...
				List<Integer> senderId = new ArrayList<Integer>();
				senderId.add(person.getPersonId());
				List<String> senderName = new ArrayList<String>();
				senderName.add(person.getMapNameLf());
				advancedSearchDocument.setSenderId(senderId);
				advancedSearchDocument.setSender(senderName);
		...
		return new ModelAndView("peoplebase/ShowSenderDocumentsPerson", model);
	}
