/*
 * ShowBirthPeoplePlaceController.java
 * 
...
import org.medici.docsources.command.geobase.ShowBirthPeoplePlaceCommand;
 ...
 */
@Controller
@RequestMapping("/de/geobase/ShowBirthPeoplePlace")
public class ShowBirthPeoplePlaceController {
	@Autowired
	...
	public ModelAndView setupForm(@ModelAttribute("requestCommand") ShowBirthPeoplePlaceCommand command, BindingResult result) {
		...
		if(command.getPlaceAllId() > 0){
			try {
				...
				outputFields.add("Birth Date");
								
				model.put("outputFields", outputFields);
				
				model.put("placeNameFull", place.getPlaceNameFull());
				model.put("bornedPeople", place.getBornedPeople());
				
			} catch (ApplicationThrowable ath) {
				new ModelAndView("error/ShowBirthPeoplePlace", model);
			}
		}

		return new ModelAndView("geobase/ShowBirthPeoplePlace", model);
	}
