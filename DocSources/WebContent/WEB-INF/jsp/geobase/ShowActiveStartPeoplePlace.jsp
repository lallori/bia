...
<%@ taglib prefix="security" uri="http://www.springframework.org/security/tags" %>
	...
	<c:url var="ShowActiveStartPeoplePlaceURL" value="/src/geobase/ShowActiveStartPeoplePlace.json"></c:url>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$j(document).ready(function() {
			...
			$j("#showActiveStartPeoplePlaceAllId${placeAllId} > thead > tr").append('<c:forEach items="${outputFields}" var="outputField"><c:out escapeXml="false" value="<th>${outputField}</th>"/></c:forEach>');

			$j('#showActiveStartPeoplePlaceAllId${placeAllId}').dataTable( {
				...
				"sAjaxSource": "${ShowActiveStartPeoplePlaceURL}",
				"sDom": 'T<"clear">lfrtip',
			...
			$j("#showActiveStartPeoplePlaceAllId${placeAllId}_length").css('margin', '0 0 0 0');
			$j("#showActiveStartPeoplePlaceAllId${placeAllId}_filter").remove();
