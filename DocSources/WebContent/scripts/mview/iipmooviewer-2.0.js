...
var IIP = new Class({
  ...
  initialize: function( main_id, options ) {
    ...
    else if( Browser.chrome || Browser.safari ) this.CSSprefix = '-webkit-';
  ...
  loadGrid: function(){
    ...
    */
	...
	// Get tile URL
	var src = this.getIIPURL( this.server, this.images[n].src, this.res, this.images[n].sds, k, i, j );
  ...
  getCVT: function(){
    ...
    var rgn = this.view.x/w + ',' + this.view.y/h + ',' + this.view.w/w + ',' + this.view.h/h;
    return this.server+'?FIF='+this.images[0].src+'&WID='+w+'&RGN='+rgn+'&CVT=jpeg';
  },
  ...
  key: function(e){
    ...
    case 82: // r
      ...
      var angle = 'rotate(' + this.orientation + 'deg)';

      this.canvas.setStyle( this.CSSprefix+'transform-origin', origin );
      this.canvas.setStyle( this.CSSprefix+'transform', angle );
//       document.id(this.source).getElement('div.navcontainer img.navimage').setStyle( this.CSSprefix+'transform', angle );
  ...
  createWindows: function(){
    ...
    if( this.annotations ){
	  ...
	  _this.canvas.getElements('div.annotation').tween( 'opacity', [0,1] );
        }
      );
	  ...
	  _this.canvas.getElements('div.annotation').tween('opacity',0);
	}
      );
    }
    ...
    if( this.disableContextMenu ){
      container.addEvent( 'contextmenu',
					 function(e){
					   var event = new Event(e);
					   event.stop();
	...
	'orientationchange': this.reload.bind(this)
	  ...
	  'touchmove': function(e){
            var event = Event(e);
	    event.stop();
	    if(e.touches.length == 1) document.id(_this.source).getElement('div.credit').set('html', 'touchmove');
          },
	  'touchend': function(e){ 
            var event = Event(e);
	    event.stop();
	    document.id(_this.source).getElement('div.credit').set('html', 'touchend:'+e.touches.length);
          },
	  'gesturestart': function(e){
	    var event = Event(e);
	    document.id(_this.source).getElement('div.credit').set('html', 'gesturestart');
	    event.stop();
	    event.preventDefault();
	  },
	  'gesturechange': function(e){
	    var event = Event(e);
	    _this.canvas.style.webkitTransform = "rotate(" + ((_this.rotating + e.rotation) % 360) + "deg)";
	    _this.canvas.setStyle( '-webkitTransform', "rotate(" + ((_this.rotating + e.rotation) % 360) + "deg)" );
	    document.id(_this.source).getElement('div.credit').set('html', 'gesturechange:'+"rotate(" + ((_this.rotating + e.rotation) % 360) + "deg)"  );
	  },
	  'gestureend': function(e){
	    if( e.scale > 1 || e.rotate > 0 ) _this.zoomIn();
	    else _this.zoomOut();
	    document.id(_this.source).getElement('div.credit').set('html', 'gestureend');
	    _this.rotating = (_this.rotating + e.rotation) % 360;
	  }
      });
    }
	...
	'events': {
	    click: function(){ container.getElement('div.info').fade(0.9); },
    ...
    // If our navigation window has been set top left, move the logo to the top right
...
//		width: this.navWin.w,
    ...
    if( this.showNavButtons ){
      ...
      var navbuttons = new Element('div', {
          'class': 'navbuttons',
	  'html': '<img class="reset" src="'+this.prefix+'reset.png"/><img class="zoomIn" src="'+this.prefix+'zoomIn.png"/><img class="zoomOut" src="'+this.prefix+'zoomOut.png"/>'
//          'html': '<img class="shiftLeft" src="'+this.prefix+'left.png"/><img class="shiftUp" src="'+this.prefix+'up.png"/><img class="shiftRight" src="'+this.prefix+'right.png"/><br/><img class="shiftDown" src="'+this.prefix+'down.png"/><br/><img class="zoomIn" src="'+this.prefix+'zoomIn.png"/><img class="zoomOut" src="'+this.prefix+'zoomOut.png"/><img class="reset" src="'+this.prefix+'reset.png"/><img class="corner" src="'+this.prefix+'corner.png"/>';
	...
	$$('div.navbuttons, div.navbuttons img').setStyle( 'opacity', 0.75 );
      ...
      navbuttons.getElement('img.zoomIn').addEvent( 'click', this.zoomIn.bind(this) );
      navbuttons.getElement('img.zoomOut').addEvent( 'click', this.zoomOut.bind(this) );
      navbuttons.getElement('img.reset').addEvent( 'click', this.reload.bind(this) );
    ...
    navcontainer.makeResizable({
      handle: document.id(this.source).getElement('div.navcontainer').getElements('img.corner'),
      onDrag: function(){
	  document.id('navigation').setStyles({
	    width: document.id(this.source).getElement('div.navcontainer').getSize().x - 10,
		height: document.id(this.source).getElement('div.navcontainer').getSize().y - 50
		});
	  preventDefault();
      }
      });*/
  ...
  updateScale: function() {
    ...
    var pixels = 1000 * this.scale * this.wid / this.max_size.w; // x1000 because we want per m
    var label = '1m';
