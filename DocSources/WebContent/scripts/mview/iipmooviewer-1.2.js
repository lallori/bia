...
var TargetDrag = new Class(
		{
			...
			initialize : function() {
				...
				this.handles = (htype == 'array' || htype == 'collection') ? $$(this.options.handle)
				...
				for ( var z in this.options.modifiers) {
					...
					if (z == 'x') {
						...
						if (iip.wid > iip.rgn_w) {
							if (iip.rgn_x - this.value.now[z] > iip.wid
									- iip.rgn_w) {
					...
					if (z == 'y') {
						...
						if (iip.hei > iip.rgn_h) {
							if (iip.rgn_y - this.value.now[z] > iip.hei
									- iip.rgn_h) {
...
var IIP = new Class(
		{
			...
			initialize : function(main_id, options) {
				...
				this.source = main_id
						|| alert('No element ID given to IIP constructor');
				...
				options['image']
						|| alert('Image location not set in IIP constructor options');
				...
				// Medici Archive Project : parametrization of help button and
				// nav thumbnail
				this.showHelpButton = options['showHelpButton'] || true;
				this.showNavThumbnail = options['showNavThumbnail'] || true;
			...
			loadGrid : function() {
				...
				if (this.wid < this.rgn_w)
					xoffset -= (this.rgn_w - this.wid) / 2;

				var yoffset = Math.floor(this.rgn_y % this.tileSize[1]);
				if (this.hei < this.rgn_h)
					yoffset -= (this.rgn_h - this.hei) / 2;
				...
				for ( var m = 0; m < ntiles; m++) {
					...
					for (n = 0; n < this.images.length; n++) {
						...
						// We set the source at the end so that the 'load'
						// function is properly fired
						var src = this.server + "?FIF=" + this.images[n].src
								+ "&cnt=" + this.contrast + "&sds="
								+ this.images[n].sds + "&jtl=" + this.res + ","
								+ k;
						tile.set('src', src);
			...
			/**
			 * Allow us to navigate within the image via the keyboard arrow
			 * buttons. Medici Archive Project customization : navigation is
			 * disabled, we insert comment on every function. If you want to
			 * enable navigate uncomment every functions.
			 */
			key : function(e) {
				var d = 100;
				switch (e.code) {
			...
			scrollNavigation : function(e) {
				...
				if (e.event) {
					...
					if ((Math.abs(xmove - this.navpos[0]) < 3)
							&& (Math.abs(ymove - this.navpos[1]) < 3))
						return;
				}

				if (xmove > (this.min_x - zone_w))
					xmove = this.min_x - zone_w;
				if (ymove > (this.min_y - zone_h))
					ymove = this.min_y - zone_h;
				if (xmove < 0)
					xmove = 0;
				if (ymove < 0)
					ymove = 0;
			...
			checkBounds : function(dx, dy) {
				...
				if (x > this.wid - this.rgn_w)
					x = this.wid - this.rgn_w;
				if (y > this.hei - this.rgn_h)
					y = this.hei - this.rgn_h;
				if (x < 0)
					x = 0;
				if (y < 0)
					y = 0;
			...
			zoomIn : function() {
				...
				if ((this.wid <= (this.max_width / 2))
						&& (this.hei <= (this.max_height / 2))) {
			...
			createWindows : function() {
				...
				// below fuction modified
				window.addEvent('resize', function() {
				});
				document.addEvent('keydown', this.key.bindWithEvent(this));

				// Medici Archive Project : Help button parametrization
				if (this.showHelpButton == true) {
					// Add our logo and a tooltip explaining how to use the
					// viewer
					new Element('a', {
					...
					new Element('img', {
						src : '/DocSources/images/mview/help.32x32.png',
						id : 'info',
						styles : {
							opacity : 0.8
						}
					...
					if (Browser.Engine.trident5) {
						$('info')
								.setStyle(
										'filter',
										'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="/DocSources/images/mview/iip.32x32.png",sizingMethod=scale)');
					}
			...
			createNavigationWindow : function() {
				...
				toolbar
						.store('tip:text',
								'* Drag to move<br/>* Double Click to show/hide navigation buttons');
				...
				// Create our navigation image and inject inside the div we just
				// created
				var navimage = new Element('img', {
					id : 'navigation',
					src : this.server + '?FIF=' + this.images[0].src + '&SDS='
							+ this.images[0].sds + '&CNT=1.0' + '&WID='
							+ this.min_x + '&QLT=99&CVT=jpeg'
				});
				...
				var navbuttons = new Element(
						...
						{
							id : 'navbuttons',
							html : '<img id="shiftLeft" src="/DocSources/images/mview/left.png"/><img id="shiftUp" src="/DocSources/images/mview/up.png"/><img id="shiftRight" src="/DocSources/images/mview/right.png"/><br/><img id="shiftDown" src="/DocSources/images/mview/down.png"/><br/><img id="zoomIn" src="/DocSources/images/mview/zoomIn.png"/><img id="zoomOut" src="/DocSources/images/mview/zoomOut.png"/><img id="reset" src="/DocSources/images/mview/reset.png"/>'
						});
				...
				if (this.showNavButtons == false)
					navbuttons.slide('out');

				// Needed as IE doesn't take CSS opacity into account
				if (Browser.Engine.trident) {
					$$('div#navbuttons, div#navbuttons img').setStyle(
							'opacity', 0.75);
				}
				...
				$('zoomOut')
						.addEvent('click', this.zoomOut.bindWithEvent(this));
				...
				$('zone').makeDraggable(
						{
							...
							// zone
							onStart : function() {
								this.navpos = [ $('zone').offsetLeft,
										$('zone').offsetTop - 10 ];
							}.bind(this),
							onComplete : this.scrollNavigation
									.bindWithEvent(this)
						});

				// Add our events
				$('navigation').addEvent('click',
						this.scrollNavigation.bindWithEvent(this));
				$('navigation').addEvent('mousewheel',
						this.zoom.bindWithEvent(this));
			...
			refreshLoadBar : function() {
				...
				$('loadBar').set(
						'html',
						'loading&nbsp;:&nbsp;'
								+ Math.round(this.nTilesLoaded
										/ this.nTilesToLoad * 100) + '%');
				...
				new Request(
						{
							...
							onComplete : function(transport) {
								...
								this.max_width = parseInt(size[0].substring(1,
										size[0].length));
								this.max_height = parseInt(size[1]);
								tmp = response.split("Tile-size");
								size = tmp[1].split(" ");
								this.tileSize[0] = parseInt(size[0].substring(
										1, size[0].length));
								this.tileSize[1] = parseInt(size[1]);
								tmp = response.split("Resolution-number");
								this.num_resolutions = parseInt(tmp[1]
										.substring(1, tmp[1].length));
			...
			positionZone : function() {
				...
				if (pleft + width > this.min_x)
					width = this.min_x - pleft;

				var height = (this.rgn_h / this.hei) * (this.min_y);
				if (height + ptop > this.min_y)
					height = this.min_y - ptop;

				if (width < this.min_x)
					this.xfit = 0;
				else
					this.xfit = 1;
				if (height < this.min_y)
					this.yfit = 0;
				else
					this.yfit = 1;
