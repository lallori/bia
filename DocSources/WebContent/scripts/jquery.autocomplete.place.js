...
(function($) {
  ...
  function AutocompletePlace(el, options) {
    this.el = $(el);
  ...
  $.fn.autocompletePlace = function(options) {
    return new AutocompletePlace(this.get(0)||$('<input />'), options);
  };


  AutocompletePlace.prototype = {
    ...
    getSuggestionsLocal: function(textFieldValue) {
      ...
      ret = { suggestions:[], data:[], prefFlags:[], plTypes:[]};
    ...
    suggest: function() {
      ...
      if (len ==0) {
	      ...
	      for (i = 0; i < len; i++) {
	        ...
	        tr = $((me.selectedIndex === i ? '<tr class="selected>"' : '<tr>') + '<td>' + f(s, this.data[i], v) + '</td>' + '<td>' + this.prefFlags[i] + '</td>' + '<td>' + this.plTypes[i] + '</td>' + '</tr>');
